...
it('blocks invites when users plus pending invites reach max_users', function () {
    ...
    expect(fn () => app(InvitationService::class)->invite($account, 'Novo', 'novo@example.com', 'user'))
        ->toThrow(ValidationException::class, 'upgrade');
});
...
it('blocks client creation checks when max_clients is reached', function () {
    ...
    expect(fn () => app(PlanLimitService::class)->ensureClientCapacity($account))
        ->toThrow(ValidationException::class, 'upgrade');
});
...
it('blocks monitoring runs when the module is locked for the plan', function () {
    ...
    expect(fn () => app(MonitoringService::class)->run($client))
        ->toThrow(ValidationException::class, 'monitoring');
});
