...
- [x] 4.1 Implementar services de checagem de limite (usuários, Clients, módulos, volume) no ponto de criação/uso, e verificar com testes de bloqueio ao estourar
- [x] 4.2 Implementar troca de Plan só pela A e área de gestão do catálogo restrita ao super_admin, e verificar com testes de permissão + smoke da tela
- [x] 4.3 Exibir aviso de limite com ação de upgrade nas telas afetadas, e verificar com `npm run build` + `npm run types:check`

## 5. Carteira de Clients

- [x] 5.1 Implementar CRUD de Clients com validação dos 4 campos e isolamento por Account, e verificar com testes de CRUD + isolamento + CNPJ repetido entre Accounts
- [x] 5.2 Implementar monitoramento agendado por Client com contagem no volume do Plan, e verificar com teste de suspensão ao esgotar volume
- [x] 5.3 Criar páginas Inertia da carteira com Nuxt UI, e verificar com `npm run build` + smoke manual

## 6. Seletor e auditoria

- [x] 6.1 Implementar seletor de Accounts (sessão switch_account_id, só super_admin, banner + saída explícita), e verificar com testes de acesso negado por papel e retorno à origem
- [x] 6.2 Implementar auditoria via observer (ator, origem, alvo, ação, momento) cobrindo plataforma e operação, e verificar com testes de registro nos eventos-chave
- [x] 6.3 Criar telas de seletor e consulta de auditoria com filtros por Account/ator, e verificar com `npm run build` + smoke manual

## 7. Validação final

- [x] 7.1 Rodar suíte completa (`php artisan test`, `npm run types:check`, `npm run build`, `npm run check`) e corrigir falhas
- [x] 7.2 Executar smoke fim a fim: onboarding → criar B → convidar admin → carteira → trocar Plan → seletor → auditoria
