...
import type { HomeChartPoint, HomePeriod } from '@/types/home';
...
function template(d: DataRecord): string {
    return `${formatDate(d.date)}: ${formatBrl(d.amount)}`;
}
</script>
...
<template>
    <UCard
        ...
        data-test="home-chart"
    >
        <template #header>
            <div>
                <p class="text-muted mb-1.5 text-xs uppercase">Receita</p>
